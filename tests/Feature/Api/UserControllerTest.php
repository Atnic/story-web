...
class UserControllerTest extends TestCase
{
    ...
    public function testStore()
    {
        ...
        $this->actingAs($user, 'api');
        $response = $this->postJson(route('api.users.store'), array_merge(factory(User::class)->make()->toArray(), [ 'password' => 'secret', 'password_confirmation' => 'secret' ]));
        if ($response->exception) {
            $this->expectOutputString('');
            $this->setOutputCallback(function () use($response) { return $response->exception; });
            return;
    ...
    public function testUpdate()
    {
        ...
        $response = $this->putJson(route('api.users.update', [ $user->getKey() ]), array_merge(factory(User::class)->make()->toArray(), [ 'password' => 'secret', 'password_confirmation' => 'secret' ]));
        if ($response->exception) {
            $this->expectOutputString('');
            $this->setOutputCallback(function () use($response) { return $response->exception; });
            return;
